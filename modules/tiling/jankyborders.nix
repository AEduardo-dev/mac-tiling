...
in
{
  options.modules.tiling.jankyborders = {
    ...
    activeColor = lib.mkOption { type = lib.types.strMatching "0x[0-9a-fA-F]{8}"; default = "0xffcba6f7"; description = "Active window border color (ARGB hex)"; };
    inactiveColor = lib.mkOption { type = lib.types.strMatching "0x[0-9a-fA-F]{8}"; default = "0xff313244"; description = "Inactive window border color (ARGB hex)"; };
    width = lib.mkOption { type = lib.types.float; default = 5.0; description = "Border width"; };
    style = lib.mkOption { type = lib.types.enum [ "round" "square" ]; default = "round"; description = "Border style (round or square)"; };
